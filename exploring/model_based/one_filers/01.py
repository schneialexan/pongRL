    ...
    def run(self):
        ...
        for e in range(self.EPISODES):
            ...
            while not done:
                ...
                self.replay()
            if e % 10 == 0:
                final_reward, frames = playOneEpisode(agent)
                imageio.mimsave(f'gifs/run2_Pongv0_{e}.gif', frames)
    ...
    def test(self, Model_name):
        ...
        for e in range(self.EPISODES):
            ...
            while not done:
                ...
                state, reward, done, _, info = self.step(action)
                score += reward
                if done:
...
def playOneEpisode(agent):
    ...
    while not done:
        ...
        observation = agent.GetImage(observation)
        action = np.argmax(agent.model.predict(observation, verbose=0))
        observation, reward, done, truncated, info = agent.env.step(action)
        totalreward += reward
        iters += 1
        if done:
            break
...
if __name__ == "__main__":
    ...
    agent.load('Models/Pong-v0_DDQN_Dueling_PER_CNN.h5')
    agent.run()
    final_reward, frames = playOneEpisode(agent)
    imageio.mimsave('gifs/Pongv0_final.gif', frames)
